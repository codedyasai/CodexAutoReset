...
{
    private const string CurrentFilePrefix = "codex-auto-reset-";
    private const string LegacyFilePrefix = "codex-reset-guard-";

    private static readonly string[] ManagedFilePrefixes =
        [CurrentFilePrefix, LegacyFilePrefix];
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var filePath = Path.Combine(
                logDirectory,
                $"{CurrentFilePrefix}{logEvent.Timestamp.UtcDateTime:yyyy-MM-dd}.jsonl");
    ...
    private void DeleteExpiredFiles(DateTime utcNow)
    {
        ...
        foreach (var filePrefix in ManagedFilePrefixes)
        {
